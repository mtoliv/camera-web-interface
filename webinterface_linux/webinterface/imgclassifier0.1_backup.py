...
def process_image(img):
	...
	lower = np.array([100, 0, 0])     # Destacar a cor verde
	upper = np.array([185, 255, 255])

	mask = cv2.inRange(hsv, lower, upper)
	# cv2.imshow('Mask', mask)

	ret, mask = cv2.threshold(mask, 60, 255, cv2.THRESH_BINARY_INV)
...
def classify_image(img,train,y = [0],img_path = [None]): #Argumento img_path adicionado por Roberto, 08/08/2019
...
def client_thread(sc,ip,port):#,q):
	...
	while True:
		...
		#            sc, info = s.accept()
			...
			if to_process: # Process this image, skip other images of the same object
				last_image = IMAGES_TO_SKIP
				valid = classify_image(mask,train = 0, y=[0]) # 0 is accepted, 1 is rejected
				print("SAVING MOST RECENT IMAGE")
